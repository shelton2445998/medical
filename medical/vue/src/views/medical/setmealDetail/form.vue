...
import {addSetmealDetail, getSetmealDetail, updateSetmealDetail} from "@/api/medical/setmealDetail";
...
const openDialog = async (id: string) => {
  ...
  dialogData.title = '新增体检套餐明细';
  if (id) {
    dialogData.id = id;
    dialogData.title = '编辑体检套餐明细';
    getDetails(id);
  }
}
...
const submit = async () => {
   ...
   if (valid) {
       let data = form.value;
     if (data.id) {
       updateSetmealDetail({...data}).then(() => {
         ElMessage.success('操作成功');
         closeDialog();
         emits('refresh');
       })
     } else {
       addSetmealDetail({...data}).then(() => {
       ElMessage.success('操作成功');
       closeDialog();
       emits('refresh');
       })
