// 定义包名，指定当前类所在的包路径
package com.fourth.medical.auth.controller;

// 导入登录注解，用于标识需要登录验证的接口
import com.fourth.medical.auth.annotation.Login;
// 导入App账号登录DTO，用于接收账号密码登录请求参数
import com.fourth.medical.auth.dto.AppAccountLoginDto;
// 导入App登录DTO，用于接收小程序登录请求参数
import com.fourth.medical.auth.dto.AppLoginDto;
// 导入App注册DTO，用于接收用户注册请求参数
import com.fourth.medical.auth.dto.AppRegisterDto;
// 导入App用户资料更新DTO，用于接收个人资料修改请求参数
import com.fourth.medical.auth.dto.AppUserUpdateProfileDto;
// 导入App登录服务接口，提供App登录相关业务逻辑
import com.fourth.medical.auth.service.AppLoginService;
// 导入App登录视图对象，用于返回App登录用户信息
import com.fourth.medical.auth.vo.AppLoginVo;
// 导入登录令牌视图对象，用于返回登录令牌信息
import com.fourth.medical.auth.vo.LoginTokenVo;
// 导入登录常量类，定义登录相关的常量值
import com.fourth.medical.common.constant.LoginConstant;
// 导入框架响应结果类，用于统一API响应格式
import com.fourth.medical.framework.response.ApiResult;
// 导入Cookie工具类，用于处理Cookie相关操作
import com.fourth.medical.util.CookieUtil;
// 导入HTTP请求工具类，用于处理请求相关操作
import com.fourth.medical.util.HttpServletRequestUtil;
// 导入Swagger操作注解，用于API文档生成
import io.swagger.v3.oas.annotations.Operation;
// 导入Swagger标签注解，用于API分组
import io.swagger.v3.oas.annotations.tags.Tag;
// 导入Lombok日志注解，自动生成日志对象
import lombok.extern.slf4j.Slf4j;
// 导入Spring自动装配注解
import org.springframework.beans.factory.annotation.Autowired;
// 导入Spring Web注解，用于REST API开发
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

// 导入Servlet HTTP请求接口
import javax.servlet.http.HttpServletRequest;
// 导入Servlet HTTP响应接口
import javax.servlet.http.HttpServletResponse;
// 导入验证注解，用于参数校验
import javax.validation.Valid;

/**
 * App登录控制器
 * 提供App端的登录、注册、获取用户信息、退出登录等功能
 * 处理App用户的身份认证和会话管理，支持小程序登录和账号密码登录
 *
 * @author geekidea
 * @date 2022/6/26
 **/
@Slf4j  // 自动生成日志对象log
@RestController  // 标识这是一个REST控制器，返回JSON格式数据
@Tag(name = "APP登录")  // Swagger文档标签，用于API分组
@RequestMapping("/app")  // 设置控制器的基础请求路径
public class AppLoginController {

    // 注入App登录服务，用于处理App登录相关业务逻辑
    @Autowired
    private AppLoginService appLoginService;

    /**
     * APP小程序登录
     * 处理小程序用户的登录请求，通过微信授权码进行身份验证
     *
     * @param appLoginDto App登录数据传输对象，包含微信授权码等信息
     * @param request HTTP请求对象，用于获取请求信息
     * @param response HTTP响应对象，用于设置Cookie
     * @return ApiResult<LoginTokenVo> 返回登录令牌的统一响应格式
     * @throws Exception 可能抛出的异常
     */
    @PostMapping("/login")  // 处理POST请求，路径为/app/login
    @Operation(summary = "APP小程序登录")  // Swagger文档说明
    public ApiResult<LoginTokenVo> login(@Valid @RequestBody AppLoginDto appLoginDto, HttpServletRequest request, HttpServletResponse response) {
        // 调用App登录服务进行小程序登录验证，获取登录令牌
        LoginTokenVo loginTokenVo = appLoginService.login(appLoginDto);
        // 将登录令牌添加到Cookie中，用于后续的身份验证
        addCookie(loginTokenVo.getToken(), request, response);
        // 返回成功响应，包含登录令牌信息
        return ApiResult.success(loginTokenVo);
    }

    /**
     * APP账号密码登录
     * 处理App用户的账号密码登录请求，验证用户名和密码
     *
     * @param loginAccountAppDto App账号登录数据传输对象，包含用户名和密码
     * @param request HTTP请求对象，用于获取请求信息
     * @param response HTTP响应对象，用于设置Cookie
     * @return ApiResult<LoginTokenVo> 返回登录令牌的统一响应格式
     * @throws Exception 可能抛出的异常
     */
    @PostMapping("/accountLogin")  // 处理POST请求，路径为/app/accountLogin
    @Operation(summary = "APP账号密码登录")  // Swagger文档说明
    public ApiResult<LoginTokenVo> accountLogin(@Valid @RequestBody AppAccountLoginDto loginAccountAppDto, HttpServletRequest request, HttpServletResponse response) {
        // 调用App登录服务进行账号密码登录验证，获取登录令牌
        LoginTokenVo loginTokenVo = appLoginService.accountLogin(loginAccountAppDto);
        // 将登录令牌添加到Cookie中，用于后续的身份验证
        addCookie(loginTokenVo.getToken(), request, response);
        // 返回成功响应，包含登录令牌信息
        return ApiResult.success(loginTokenVo);
    }

    /**
     * APP用户注册
     * 处理App用户的新用户注册请求，创建用户账号并自动登录
     *
     * @param appRegisterDto App注册数据传输对象，包含用户注册信息
     * @param request HTTP请求对象，用于获取请求信息
     * @param response HTTP响应对象，用于设置Cookie
     * @return ApiResult<LoginTokenVo> 返回登录令牌的统一响应格式
     * @throws Exception 可能抛出的异常
     */
    @PostMapping("/register")  // 处理POST请求，路径为/app/register
    @Operation(summary = "APP用户注册")  // Swagger文档说明
    public ApiResult<LoginTokenVo> register(@Valid @RequestBody AppRegisterDto appRegisterDto, HttpServletRequest request, HttpServletResponse response) {
        // 调用App登录服务进行用户注册，注册成功后自动登录并获取令牌
        LoginTokenVo loginTokenVo = appLoginService.register(appRegisterDto);
        // 将登录令牌添加到Cookie中，用于后续的身份验证
        addCookie(loginTokenVo.getToken(), request, response);
        // 返回成功响应，包含登录令牌信息
        return ApiResult.success(loginTokenVo);
    }

    /**
     * 获取APP登录用户信息
     * 获取当前登录App用户的详细信息，需要登录验证
     *
     * @return ApiResult<AppLoginVo> 返回App登录用户信息的统一响应格式
     * @throws Exception 可能抛出的异常
     */
    @Login  // 需要登录验证的接口
    @PostMapping("/getLoginUserInfo")  // 处理POST请求，路径为/app/getLoginUserInfo
    @Operation(summary = "获取APP登录用户信息")  // Swagger文档说明
    public ApiResult<AppLoginVo> getLoginUserInfo() {
        // 调用App登录服务获取当前登录用户的详细信息
        AppLoginVo appLoginVo = appLoginService.getLoginUserInfo();
        // 返回成功响应，包含用户信息
        return ApiResult.success(appLoginVo);
    }

    /**
     * APP退出
     * 处理App用户的退出登录请求，清除登录状态和缓存
     *
     * @param request HTTP请求对象，用于获取请求信息
     * @param response HTTP响应对象，用于删除Cookie
     * @return ApiResult<Boolean> 返回退出结果的统一响应格式
     * @throws Exception 可能抛出的异常
     */
    @PostMapping("/logout")  // 处理POST请求，路径为/app/logout
    @Operation(summary = "APP退出")  // Swagger文档说明
    public ApiResult<Boolean> logout(HttpServletRequest request, HttpServletResponse response) {
        // 调用App登录服务清除登录缓存和会话信息
        appLoginService.logout();
        // 从Cookie中删除登录令牌，清除客户端的登录状态
        CookieUtil.deleteCookie(LoginConstant.APP_COOKIE_TOKEN_NAME, request, response);
        // 返回成功响应
        return ApiResult.success();
    }

    /**
     * 添加Cookie
     * 将登录令牌添加到Cookie中，用于客户端身份验证
     *
     * @param token 登录令牌字符串
     * @param request HTTP请求对象，用于获取请求信息
     * @param response HTTP响应对象，用于设置Cookie
     */
    private void addCookie(String token, HttpServletRequest request, HttpServletResponse response) {
        // 使用Cookie工具类将登录令牌添加到Cookie中
        CookieUtil.addCookie(LoginConstant.APP_COOKIE_TOKEN_NAME, token, request, response);
    }

    /**
     * 修改用户个人信息（User表）
     * 更新当前登录App用户的个人资料信息，需要登录验证
     *
     * @param dto App用户资料更新DTO，包含要修改的用户信息
     * @return ApiResult 返回操作结果的统一响应格式
     * @throws Exception 可能抛出的异常
     */
    @Login  // 需要登录验证的接口
    @PostMapping("/updateUserProfile")  // 处理POST请求，路径为/app/updateUserProfile
    @Operation(summary = "修改用户个人信息")  // Swagger文档说明
    public ApiResult updateUserProfile(@Valid @RequestBody AppUserUpdateProfileDto dto) {
        // 调用App登录服务更新用户个人资料，@Valid注解用于参数校验
        boolean flag = appLoginService.updateAppUserProfile(dto);
        // 根据操作结果返回相应的响应
        return ApiResult.result(flag);
    }

}
