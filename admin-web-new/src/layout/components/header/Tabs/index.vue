...
<template>
  ...
  >
    ...
    >
      <template #label>
        <el-dropdown ref="dropdownRef" @visible-change="(value)=>change(index,value)" trigger="contextmenu" placement="bottom-end">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              ...
              <el-dropdown-item :disabled="index<=1" :icon="Back" @click="closeLeft(item)">关闭左侧</el-dropdown-item>
              <el-dropdown-item :disabled="index>=tabsList.length-1" :icon="Right" @click="closeRight(item)">关闭右侧</el-dropdown-item>
              <el-dropdown-item :disabled="tabsList.length<=2" :icon="Switch" @click="closeOther(item)">关闭其他</el-dropdown-item>
...
const change = (index:number,value:boolean) => {
  if(value){
    dropdownRef.value.forEach((p:any,i:number)=>{
      if(index!=i){
        p.handleClose();
      }
    })
...
let activeKey = tabsStore.getCurrentTab?.name;
