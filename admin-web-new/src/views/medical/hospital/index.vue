<template>
  <el-card shadow="never" class="card-box">
    ...
    <div v-auth="'hospital:add'" class="table-btn-box mb10">
    ...
    <el-table v-loading="tableLoading.status" :data="tableData.data" border row-key="id" @sort-change="sortChange">
      ...
      <el-table-column prop="name" label="医院名称" align="center" show-overflow-tooltip/>
      <el-table-column prop="phone" label="医院电话" align="center" show-overflow-tooltip/>
      <el-table-column prop="address" label="医院地址" align="center" show-overflow-tooltip/>
      <el-table-column prop="description" label="医院简介" align="center" show-overflow-tooltip/>
      ...
      <el-table-column label="操作" fixed="right" align="center" min-width="116">
        <template #default="scope">
          <el-button v-auth="'hospital:update'" link type="primary" @click="openDialog(scope.row.id)">
            <el-icon>
              <ele-edit/>
            </el-icon>
            修改
          </el-button>
          <el-button v-auth="'hospital:delete'" link type="primary" @click="delTable(scope.row.id)">
            <el-icon>
              <ele-delete/>
            </el-icon>
            删除
...
import {deleteHospital, getHospitalPage} from "@/api/medical/hospital";
...
const getTableList = () => {
    ...
    getHospitalPage({...pagination, ...queryForm.value, ...orderBy.value}).then(res => {
