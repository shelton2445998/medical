<template>
  <el-card shadow="never" class="card-box">
    ...
    <div v-auth="'report:item:add'" class="table-btn-box mb10">
    ...
    <el-table v-loading="tableLoading.status" :data="tableData.data" border row-key="id" @sort-change="sortChange">
      ...
      <el-table-column prop="reportStatus" label="报告状态" align="center">
        <template #default="scope">
          <span v-if="scope.row.reportStatus == 0">未生成</span>
          <span v-if="scope.row.reportStatus == 1">已生成</span>
        </template>
      </el-table-column>
      <el-table-column prop="conclusion" label="检查结论" align="center" show-overflow-tooltip/>
      ...
      <el-table-column label="操作" fixed="right" align="center" min-width="116">
        <template #default="scope">
          <el-button v-auth="'report:item:update'" link type="primary" @click="openDialog(scope.row.id)">
            <el-icon>
              <ele-edit/>
            </el-icon>
            修改
          </el-button>
          <el-button v-auth="'report:item:delete'" link type="primary" @click="delTable(scope.row.id)">
            <el-icon>
              <ele-delete/>
            </el-icon>
            删除
...
import {deleteReportItem, getReportItemPage} from "@/api/medical/reportItem";
...
const getTableList = () => {
    ...
    getReportItemPage({...pagination, ...queryForm.value, ...orderBy.value}).then(res => {
