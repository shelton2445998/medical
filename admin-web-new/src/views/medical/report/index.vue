<template>
  <el-card shadow="never" class="card-box">
    ...
    <div v-auth="'report:add'" class="table-btn-box mb10">
    ...
    <el-table v-loading="tableLoading.status" :data="tableData.data" border row-key="id" @sort-change="sortChange">
      ...
      <el-table-column prop="checkitemIds" label="检查项ID列表" align="center" show-overflow-tooltip/>
      <el-table-column prop="reportItemIds" label="报告项ID列表" align="center" show-overflow-tooltip/>
      ...
      <el-table-column prop="conclusion" label="总结论" align="center" show-overflow-tooltip/>
      ...
      <el-table-column label="操作" fixed="right" align="center" min-width="116">
        <template #default="scope">
          <el-button v-auth="'report:update'" link type="primary" @click="openDialog(scope.row.id)">
            <el-icon>
              <ele-edit/>
            </el-icon>
            修改
          </el-button>
          <el-button v-auth="'report:delete'" link type="primary" @click="delTable(scope.row.id)">
            <el-icon>
              <ele-delete/>
            </el-icon>
            删除
...
import {deleteReport, getReportPage} from "@/api/medical/report";
...
const getTableList = () => {
    ...
    getReportPage({...pagination, ...queryForm.value, ...orderBy.value}).then(res => {
