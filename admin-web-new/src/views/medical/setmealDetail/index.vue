<template>
  <el-card shadow="never" class="card-box">
    ...
    <div v-auth="'setmeal:detail:add'" class="table-btn-box mb10">
    ...
    <el-table v-loading="tableLoading.status" :data="tableData.data" border row-key="id" @sort-change="sortChange">
      ...
      <el-table-column label="操作" fixed="right" align="center" min-width="116">
        <template #default="scope">
          <el-button v-auth="'setmeal:detail:update'" link type="primary" @click="openDialog(scope.row.id)">
            <el-icon>
              <ele-edit/>
            </el-icon>
            修改
          </el-button>
          <el-button v-auth="'setmeal:detail:delete'" link type="primary" @click="delTable(scope.row.id)">
            <el-icon>
              <ele-delete/>
            </el-icon>
            删除
...
const getTableList = () => {
    ...
    getSetmealDetailPage({...pagination, ...queryForm.value, ...orderBy.value}).then(res => {
