<template>
  <el-card shadow="never" class="card-box">
    ...
    <div v-auth="'doctor:add'" class="table-btn-box mb10">
    ...
    <el-table v-loading="tableLoading.status" :data="tableData.data" border row-key="id" @sort-change="sortChange">
      ...
      <el-table-column prop="name" label="医生姓名" align="center" show-overflow-tooltip/>
      ...
      <el-table-column prop="hospitalId" label="所属医院ID" align="center" show-overflow-tooltip/>
      <el-table-column prop="departmentId" label="科室ID" align="center" show-overflow-tooltip/>
      <el-table-column prop="title" label="职称" align="center" show-overflow-tooltip/>
      ...
      <el-table-column label="操作" fixed="right" align="center" min-width="116">
        <template #default="scope">
          <el-button v-auth="'doctor:update'" link type="primary" @click="openDialog(scope.row.id)">
            <el-icon>
              <ele-edit/>
            </el-icon>
            修改
          </el-button>
          <el-button v-auth="'doctor:delete'" link type="primary" @click="delTable(scope.row.id)">
            <el-icon>
              <ele-delete/>
            </el-icon>
            删除
...
import {deleteDoctor, getDoctorPage} from "@/api/medical/doctor";
...
const getTableList = () => {
    ...
    getDoctorPage({...pagination, ...queryForm.value, ...orderBy.value}).then(res => {
