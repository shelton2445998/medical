<template>
  <el-card shadow="never" class="card-box">
    ...
    <div v-auth="'orders:add'" class="table-btn-box mb10">
    ...
    <el-table v-loading="tableLoading.status" :data="tableData.data" border row-key="id" @sort-change="sortChange">
      ...
      <el-table-column prop="setmealId" label="套餐ID" align="center" show-overflow-tooltip/>
      <el-table-column prop="hospitalId" label="医院ID" align="center" show-overflow-tooltip/>
      ...
      <el-table-column prop="amount" label="订单金额" align="center" show-overflow-tooltip/>
      ...
      <el-table-column prop="transactionId" label="支付交易号" align="center" show-overflow-tooltip/>
      ...
      <el-table-column label="操作" fixed="right" align="center" min-width="116">
        <template #default="scope">
          <el-button v-auth="'orders:update'" link type="primary" @click="openDialog(scope.row.id)">
            <el-icon>
              <ele-edit/>
            </el-icon>
            修改
          </el-button>
          <el-button v-auth="'orders:delete'" link type="primary" @click="delTable(scope.row.id)">
            <el-icon>
              <ele-delete/>
            </el-icon>
            删除
...
import {deleteOrders, getOrdersPage} from "@/api/medical/orders";
...
const getTableList = () => {
    ...
    getOrdersPage({...pagination, ...queryForm.value, ...orderBy.value}).then(res => {
