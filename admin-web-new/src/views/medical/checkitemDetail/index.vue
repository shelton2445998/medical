<template>
  <el-card shadow="never" class="card-box">
    ...
    <div v-auth="'checkitem:detail:add'" class="table-btn-box mb10">
    ...
    <el-table v-loading="tableLoading.status" :data="tableData.data" border row-key="id" @sort-change="sortChange">
      ...
      <el-table-column prop="itemId" label="检查项ID" align="center" show-overflow-tooltip/>
      <el-table-column prop="name" label="明细名称" align="center" show-overflow-tooltip/>
      <el-table-column prop="normalValue" label="正常值范围" align="center" show-overflow-tooltip/>
      ...
      <el-table-column label="操作" fixed="right" align="center" min-width="116">
        <template #default="scope">
          <el-button v-auth="'checkitem:detail:update'" link type="primary" @click="openDialog(scope.row.id)">
            <el-icon>
              <ele-edit/>
            </el-icon>
            修改
          </el-button>
          <el-button v-auth="'checkitem:detail:delete'" link type="primary" @click="delTable(scope.row.id)">
            <el-icon>
              <ele-delete/>
            </el-icon>
            删除
...
import {deleteCheckitemDetail, getCheckitemDetailPage} from "@/api/medical/checkitemDetail";
...
const getTableList = () => {
    ...
    getCheckitemDetailPage({...pagination, ...queryForm.value, ...orderBy.value}).then(res => {
