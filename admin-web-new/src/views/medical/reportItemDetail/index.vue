<template>
  <el-card shadow="never" class="card-box">
    ...
    <div v-auth="'report:item:detail:add'" class="table-btn-box mb10">
    ...
    <el-table v-loading="tableLoading.status" :data="tableData.data" border row-key="id" @sort-change="sortChange">
      ...
      <el-table-column prop="reportId" label="报告ID" align="center" show-overflow-tooltip/>
      <el-table-column prop="detailId" label="检查项明细ID" align="center" show-overflow-tooltip/>
      <el-table-column prop="value" label="检查值" align="center" show-overflow-tooltip/>
      <el-table-column prop="isAbnormal" label="是否异常" align="center">
        <template #default="scope">
          <span v-if="scope.row.isAbnormal == 0">正常</span>
          <span v-if="scope.row.isAbnormal == 1">异常</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" fixed="right" align="center" min-width="116">
        <template #default="scope">
          <el-button v-auth="'report:item:detail:update'" link type="primary" @click="openDialog(scope.row.id)">
            <el-icon>
              <ele-edit/>
            </el-icon>
            修改
          </el-button>
          <el-button v-auth="'report:item:detail:delete'" link type="primary" @click="delTable(scope.row.id)">
            <el-icon>
              <ele-delete/>
            </el-icon>
            删除
...
import {deleteReportItemDetail, getReportItemDetailPage} from "@/api/medical/reportItemDetail";
...
const getTableList = () => {
    ...
    getReportItemDetailPage({...pagination, ...queryForm.value, ...orderBy.value}).then(res => {
...
const delTable = (id: string) => {
    ElMessageBox.confirm(
            ...
            .then(() => {
                deleteReportItemDetail(id).then(() => {
