<template>
  <el-card shadow="never" class="card-box">
    ...
    <div v-auth="'setmeal:add'" class="table-btn-box mb10">
    ...
    <el-table v-loading="tableLoading.status" :data="tableData.data" border row-key="id" @sort-change="sortChange">
      ...
      <el-table-column prop="type" label="套餐类型" align="center">
        <template #default="scope">
          ...
          <span v-if="scope.row.type == 2">高级</span>
          <span v-if="scope.row.type == 3">专项</span>
        </template>
      </el-table-column>
      <el-table-column prop="price" label="价格" align="center" show-overflow-tooltip/>
      <el-table-column prop="discountPrice" label="折扣价" align="center" show-overflow-tooltip/>
      <el-table-column prop="description" label="描述" align="center" show-overflow-tooltip/>
      <el-table-column prop="status" label="状态" align="center">
        <template #default="scope">
          <el-tag v-if="scope.row.status == false" type="danger" disable-transitions>下架</el-tag>
      ...
      <el-table-column label="操作" fixed="right" align="center" min-width="116">
        <template #default="scope">
          <el-button v-auth="'setmeal:update'" link type="primary" @click="openDialog(scope.row.id)">
            <el-icon>
              <ele-edit/>
            </el-icon>
            修改
          </el-button>
          <el-button v-auth="'setmeal:delete'" link type="primary" @click="delTable(scope.row.id)">
            <el-icon>
              <ele-delete/>
            </el-icon>
            删除
...
import {deleteSetmeal, getSetmealPage} from "@/api/medical/setmeal";
...
const getTableList = () => {
    ...
    getSetmealPage({...pagination, ...queryForm.value, ...orderBy.value}).then(res => {
