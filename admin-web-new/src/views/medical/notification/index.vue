<template>
  <el-card shadow="never" class="card-box">
    ...
    <div v-auth="'notification:add'" class="table-btn-box mb10">
    ...
    <el-table v-loading="tableLoading.status" :data="tableData.data" border row-key="id" @sort-change="sortChange">
      ...
      <el-table-column prop="userId" label="接收用户ID" align="center" show-overflow-tooltip/>
      <el-table-column prop="doctorId" label="接收医生ID" align="center" show-overflow-tooltip/>
      <el-table-column prop="title" label="通知标题" align="center" show-overflow-tooltip/>
      ...
      <el-table-column prop="type" label="通知类型" align="center">
        <template #default="scope">
          <span v-if="scope.row.type == 1">预约提醒</span>
          <span v-if="scope.row.type == 2">报告通知</span>
          <span v-if="scope.row.type == 3">系统公告</span>
        </template>
      </el-table-column>
      <el-table-column prop="isRead" label="是否已读" align="center">
        <template #default="scope">
          <el-tag v-if="scope.row.isRead == false" type="danger" disable-transitions>未读</el-tag>
          <el-tag v-if="scope.row.isRead == true" type="success" disable-transitions>已读</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" fixed="right" align="center" min-width="116">
        <template #default="scope">
          <el-button v-auth="'notification:update'" link type="primary" @click="openDialog(scope.row.id)">
            <el-icon>
              <ele-edit/>
            </el-icon>
            修改
          </el-button>
          <el-button v-auth="'notification:delete'" link type="primary" @click="delTable(scope.row.id)">
            <el-icon>
              <ele-delete/>
            </el-icon>
            删除
...
import {deleteNotification, getNotificationPage} from "@/api/medical/notification";
...
const getTableList = () => {
    ...
    getNotificationPage({...pagination, ...queryForm.value, ...orderBy.value}).then(res => {
