<template>
  <el-card shadow="never" class="card-box">
    ...
    <el-table :data="tableData" border style="width: 100%" v-loading="loading">
      ...
      <el-table-column prop="checkItemCount" label="包含项目数" width="100" align="center" />
    ...
    <el-dialog
      ...
      append-to-body
    >
      <el-form ref="packageForm" :model="packageForm" :rules="formRules" label-width="100px">
...
const getPackageList = async () => {
  ...
  try {
    ...
    const res = await http.get('/admin/examination/package/list', { params });
...
const getCheckItemsList = async () => {
  try {
    const res = await http.get('/admin/examination/check-item/all');
...
const openAddForm = () => {
  ...
  Object.keys(packageForm).forEach(key => {
    packageForm[key] = key === 'status' ? 1 : key === 'price' ? 0 : key === 'checkItems' ? [] : '';
  });
...
const editPackage = async (row) => {
  formType.value = 'edit';
  try {
    const res = await http.get(`/admin/examination/package/detail/${row.id}`);
    Object.keys(packageForm).forEach(key => {
...
const viewItems = async (row) => {
  try {
    const res = await http.get(`/admin/examination/package/items/${row.id}`);
...
const changeStatus = async (row) => {
  try {
    await http.put(`/admin/examination/package/status/${row.id}/${row.status === 1 ? 0 : 1}`);
...
const deletePackage = (row) => {
  ElMessageBox.confirm('确定要删除该套餐吗？', '提示', {
    ...
    try {
      await http.delete(`/admin/examination/package/${row.id}`);
...
const submitForm = async () => {
  try {
    await packageForm.value.validate();
    const url = formType.value === 'add' 
      ? '/admin/examination/package/add' 
      : '/admin/examination/package/update';
    await http.post(url, packageForm);
    ElMessage.success(`${formType.value === 'add' ? '添加' : '更新'}成功`);
