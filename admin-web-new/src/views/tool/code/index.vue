...
<template>
  <el-card shadow="never" class="card-box">
    ...
    <el-dialog
        ...
        width="80%"
        fullscreen
        @close="preview.isShow=false"
    >
      <el-tabs v-model="preview.activeName">
        <el-tab-pane v-for="item in preview.data" :label="item.fileName" :name="item.fileName">
...
// 获取表格列表
const getTableList = () => {
  getDbTablePageApi({...pagination, ...queryForm.value}).then(res => {
...
// 跳转配置页面
const jumpConfiguration = (tableName: string) => {
  router.push({
...
// 同步表信息
const syncTableInfo = (tableName: string) => {

}

// 生成代码
const downloadCode = async (tableNames?: string[], generatorType?: number) => {
  if (tableNames === undefined) {
    tableNames = tableRef.value.getSelectionRows().map((p: { tableName: string }) => p.tableName)
  }
...
// 预览
const previewCode = (tableName: string) => {
